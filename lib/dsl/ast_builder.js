    ...
    optionDeclarationNames.forEach(optionDeclarationName => {
      ...
      context[optionDeclarationName].map(this.visit, this).forEach(parsedOption => {
        ...
        if (parsedOption.value) {
          const alreadyInTheAst = ast.options[optionName][parsedOption.value];
          if (!alreadyInTheAst) {
            optionInTheAst = { list: parsedOption.list, excluded: [] };
            ast.options[optionName][parsedOption.value] = optionInTheAst;
          } else {
            optionInTheAst = ast.options[optionName][parsedOption.value];
          }
        } else {
          optionInTheAst = ast.options[optionName];
        }

        const mergeEntityListsWithoutDuplicates = (firstList, secondList) =>
          Array.from(new Set(firstList.concat(secondList)));
