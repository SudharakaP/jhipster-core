...
const optionDeclarationNameToOptionNameMapping = {
  dtoDeclaration: 'dto',
...
class JDLAstBuilderVisitor extends BaseJDLCSTVisitor {
  ...
  prog(context) {
    ...
    ast.options = this.getOptionsInTheAst(context);

    return ast;
  }

  getOptionsInTheAst(context) {
    const options = {
      dto: {},
      pagination: {},
      service: {},
    ...
    const optionDeclarationNames = Object.keys(optionDeclarationNameToOptionNameMapping);
    optionDeclarationNames.forEach(optionDeclarationName => {
      const optionName = optionDeclarationNameToOptionNameMapping[optionDeclarationName];
      if (!context[optionDeclarationName]) {
        return;
      }
      context[optionDeclarationName].map(this.visit, this).forEach(parsedOption => {
        let optionInTheAst;
        if (parsedOption.value) {
          const alreadyInTheAst = options[optionName][parsedOption.value];
          if (!alreadyInTheAst) {
            optionInTheAst = { list: parsedOption.list, excluded: [] };
            options[optionName][parsedOption.value] = optionInTheAst;
          } else {
            optionInTheAst = options[optionName][parsedOption.value];
          }
        } else {
          optionInTheAst = options[optionName];
        }

        optionInTheAst.list = mergeEntityListsWithoutDuplicates(optionInTheAst.list, parsedOption.list);

        if (parsedOption.excluded) {
          optionInTheAst.excluded = mergeEntityListsWithoutDuplicates(optionInTheAst.excluded, parsedOption.excluded);
        }
      });
    });
    return options;
  ...
  dtoDeclaration(context) {
    return getBinaryOptionFromContext({ context, visitor: this });
  }
  ...
  skipClientDeclaration(context) {
    return extractUnaryOptionFromContext({ context, visitor: this });
  }
  ...
  applicationSubDeclaration(context) {
    ...
    if (context.applicationEntityOptions) {
      applicationSubDeclaration.options = this.visit(context.applicationEntityOptions);
    }
  ...
  applicationSubEntities(context) {
    const result = extractUnaryOptionFromContext({ context, visitor: this, listPropName: 'entityList' });
    if (!result.excluded) {
      result.excluded = [];
    }
    return result;
  ...
  applicationEntityOptions(context) {
    return {};
  }
...
function getBinaryOptionFromContext({ context, visitor }) {
  ...
  return { value: last, list: allButLast, excluded: exclusion };
}

function extractUnaryOptionFromContext({ context, visitor, listPropName }) {
  if (!listPropName) {
    listPropName = 'list';
  }
...
function mergeEntityListsWithoutDuplicates(firstList, secondList) {
