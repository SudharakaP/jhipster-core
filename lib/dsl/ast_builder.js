...
class JDLAstBuilderVisitor extends BaseJDLCSTVisitor {
  ...
  prog(context) {
    ...
    optionDeclarationNames.forEach(optionDeclarationName => {
      ...
      context[optionDeclarationName].map(this.visit, this).forEach(parsedOption => {
        let optionAst;
        if (parsedOption.name) {
          const alreadyInTheAst = ast.options[optionName][parsedOption.name];
          if (!alreadyInTheAst) {
            optionAst = { list: parsedOption.list, excluded: [] };
            ast.options[optionName][parsedOption.name] = optionAst;
          } else {
            optionAst = ast.options[optionName][parsedOption.name];
          }
        } else {
  ...
  dtoDeclaration(context) {
    return extractNameListExcluded({ context, visitor: this });
  }
  ...
  skipClientDeclaration(context) {
    return extractListExcluded({ context, visitor: this });
  }
  ...
  applicationSubEntities(context) {
    const result = extractListExcluded({ context, visitor: this, listPropName: 'entityList' });
    if (!result.excluded) {
      result.excluded = [];
    }
    return result;
