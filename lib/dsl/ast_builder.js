    ...
    optionDeclarationNames.forEach(optionDeclarationName => {
      ...
      context[optionDeclarationName].map(this.visit, this).forEach(parsedOption => {
        ...
        if (parsedOption.value) {
          ...
          if (!alreadyInTheAst) {
            optionAst = { list: parsedOption.list, excluded: [] };
            ast.options[optionName][parsedOption.value] = optionAst;
          } else {
            optionAst = ast.options[optionName][parsedOption.value];
          }
        } else {
          optionAst = ast.options[optionName];
        }

        optionAst.list = Array.from(new Set(optionAst.list.concat(parsedOption.list)));

        if (parsedOption.excluded) {
          optionAst.excluded = Array.from(new Set(optionAst.excluded.concat(parsedOption.excluded)));
        }
      });
    });
