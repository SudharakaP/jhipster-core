  ...
  prog(context) {
    ...
    optionDeclarationNames.forEach(optionDeclarationName => {
      ...
      context[optionDeclarationName].map(this.visit, this).forEach(parsedOption => {
        ...
        optionInTheAst.list = Array.from(new Set(optionInTheAst.list.concat(parsedOption.list)));

        if (parsedOption.excluded) {
          optionInTheAst.excluded = Array.from(new Set(optionInTheAst.excluded.concat(parsedOption.excluded)));
        }
      });
    });
